  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-gray-600">
        ...
        <code className="rounded bg-gray-100 px-1">Authorization: Bearer &lt;token&gt;</code>.
      </p>
      ...
      {token && (
        <div className="rounded-md border border-green-200 bg-green-50 p-3">
          ...
          <div className="flex items-center gap-2">
            <code className="flex-1 break-all rounded bg-white px-2 py-1 text-xs">{token}</code>
            <button
      ...
      <div className="flex items-center gap-3">
        ...
        {!token && exists && (
          <span className="text-sm text-gray-500">A token is currently active.</span>
        )}
      </div>
    </div>
  );
